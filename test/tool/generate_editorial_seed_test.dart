...
import 'package:atlas/features/prices/data/price_observation_catalog_sql.dart';
...
void main() {
  test('génère supabase/seed.sql depuis les catalogues Dart', () {
    PriceObservationCatalogValidator.assertValid();

    final buffer = StringBuffer()
      ..writeln('-- Généré par test/tool/generate_editorial_seed_test.dart')
      ..writeln(
        '-- =============================================================================',
      )
...
''')
    ...
    for (final event in EventCatalog.fixedPublicHolidaysForYears([
      2026,
      2027,
    ])) {
    ...
    buffer.writeln();
    // Price Intelligence Wave 1 — observations vérifiées (sources opérateurs).
    for (final entry in PriceObservationCatalog.entries) {
      buffer.writeln(PriceObservationCatalogSql.upsertOne(entry));
    }
  ...
  test('génère migration additive locale 00015 (catalogue complet local)', () {
    PriceObservationCatalogValidator.assertValid();
    final sql = PriceObservationCatalogSql.upsertAll(
      headerComment: '''
-- =============================================================================
...
-- =============================================================================''',
    ...
    File(
      'supabase/migrations/00015_seed_verified_price_observations.sql',
    ).writeAsStringSync(sql);
  });

  test('génère migration additive 00019 Wave 2 culture Marrakech', () {
    PriceObservationCatalogValidator.assertValid();
    final culture = PriceObservationCatalog.entries
...
''')
      ..writeln('BEGIN;')
      ..writeln();
    for (final entry in culture) {
      buffer.writeln(PriceObservationCatalogSql.upsertOne(entry));
      buffer.writeln();
    }
    buffer.writeln('COMMIT;');
    File(
      'supabase/migrations/00019_prices_wave2_marrakech_culture.sql',
